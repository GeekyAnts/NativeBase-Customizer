...
console.error = function(arg, ...args) {
  if (
    ...
    (arg.startsWith("Warning: React does not recognize the") ||
      ...
      arg.startsWith(
        "Warning: Received `true` for non-boolean attribute `accessible`"
      ) ||
      arg.startsWith("Warning: Stateless function components cannot") ||
      arg.startsWith(
        "Warning: Failed prop type: Invalid props.style key `pointerEvents` supplied to `View`."
      ) ||
      arg.match(
        /Warning: Received `.*` for non-.* attribute `.*`\. If this is expected, cast the value to a .*\./
      ))
  ) {
    return;
  }
